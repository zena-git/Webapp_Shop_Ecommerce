    ...
    return (
        <div className="flex flex-col gap-5 bg-white px-5 py-3">
            ...
            <div className="grid grid-cols-2 grid-rows-2 grid-flow-row gap-2 bg-slate-50 p-6 rounded-md shadow-lg">
                ...
                <p className="text-xl font-semibold text-slate-800">{targetVoucher?.usage_limit || "Không có"}</p>
                <p className="text-xl font-semibold text-slate-800">Giá trị giảm:</p>
                <p className="text-xl font-semibold text-slate-800">{targetVoucher?.value} {targetVoucher && targetVoucher.discount_type == 0 ? "đ" : "%"}</p>
                {targetVoucher?.order_min_value && <p className="text-xl font-semibold text-slate-800">Giá trị đơn tối thiểu: </p>}
                {targetVoucher?.order_min_value && <p className="text-xl font-semibold text-slate-800">{targetVoucher?.order_min_value}</p>}
                {targetVoucher.discount_type == 1 && <p className="text-xl font-semibold text-slate-800">Mức giảm tối đa:</p>}
